...
using namespace ns3;
std::vector<Time> generationTimes;
std::vector<Time> receptionTimes;
std::vector<float> pageLoadTimes;
...
MainObjectGenerated(uint32_t size)
{
    ...
    Time generationTime = Simulator::Now();
...
ClientMainObjectReceived(Ptr<const ThreeGppHttpClient>, Ptr<const Packet> packet)
{
        ...
        header.GetContentType() == ThreeGppHttpHeader::MAIN_OBJECT)
    {
        ...
        receptionTimes.push_back(receptionTime);
    }
    else
    {
...
ClientPageReceived(Ptr<const ThreeGppHttpClient> client,
                   ...
                   uint32_t numBytes)
{
    // std::cout << "Client " << client << " has received a page that took " << time.As(Time::MS)
    //           << " ms to load with " << numObjects << " objects and " << numBytes << " bytes."
    //           << std::endl;
    std::ostringstream output2;
    // output << "Client " << client << " has received a page that took " << time.As(Time::MS)
    //        << " ms to load with " << numObjects << " objects and " << numBytes << " bytes.";

    output2 << time.As(Time::MS);

    std::string outputString = output2.str();
    // std::cout<< outputString<<std::endl;

    // std::cout<< std::stof(outputString.substr(1,5)) <<std::endl;
    pageLoadTimes.push_back(std::stof(outputString.substr(1, 6)));
}

ApplicationContainer serverApps;

void createMyServer(auto serverNodes){
    // Create HTTP server helper
    ThreeGppHttpServerHelper serverHelper(Ipv4Address::GetAny());

    // Install HTTP server
    serverApps = serverHelper.Install(serverNodes.get());
    // std::cout << serverApps.GetN() << " Hellc" << std::endl;

    // one for loop if sta as servers
    ...
    httpVariables->SetMainObjectSizeMean(114);  
    httpVariables->SetMainObjectSizeStdDev(268); 
    httpVariables->SetReadingTimeMean(Seconds(1));
    httpVariables->SetNumOfEmbeddedObjectsMax(110);
    httpVariables->SetEmbeddedObjectSizeMean(60);
    httpVariables->SetEmbeddedObjectSizeStdDev(968);

    //}
    //std::cout << httpVariables->GetNumOfEmbeddedObjects() << std::endl;
    // serverApps.Start(Seconds(0.0));
    // serverApps.Stop(Seconds(simulationTime + 1));
}

void createMyClient(size_t nStations,Ipv4InterfaceContainer serverInterfaces,double simulationTime,NodeContainer clientNodes){
  for (std::size_t i = 0; i < nStations; i++)
    {
        // Create HTTP client helper
        ThreeGppHttpClientHelper clientHelper(serverInterfaces.GetAddress(i));
        ...
        //std::cout << i << " Hell" << std::endl;
        // Stop browsing after 30 minutes
        clientApps.Start(Seconds(1.0));
        clientApps.Stop(Seconds(simulationTime + 1));
        // std::cout << clientApps.GetN() << " Hellc" << std::endl;
    }
}

void Ultimate(std::string SeqType);
...
private:
  ...
  double m_startInterval {0};        // duration of the interval in which apps can start sending packets (ms)
  std::vector<uint64_t> m_rngValues {1, 40}; // values to initialize the RNG
  uint16_t m_nHeStations {10};
  ...
  std::string m_heRate {"11"};       // "ideal" or MCS value
  ...
  for (uint16_t bss = 0; bss < m_nObss + 1; bss++)
    {
      ...
      if (m_enablePcap)
        {
          phy.EnablePcap ("scratch/Q1Pcap/OFDM/STA_pcap", staDevices);
          phy.EnablePcap ("scratch/Q1Pcap/OFDM/AP_pcap", apDevice);
        }
    }
...
void Ultimate(std::string SeqType)
{
    ...
    uint32_t payloadSize =
        1000; // must fit in the max TX duration when transmitting at MCS 0 over an RU of 26 tones
    ...
    if(SeqType=="ACK-SU-FORMAT"){
      dlAckSeqType="ACK-SU-FORMAT";
    }
    else if(SeqType=="NO-OFDMA"){
      dlAckSeqType="NO-OFDMA";
    }
    ...
    if (!udp)
    {
        ...
        Config::SetDefault("ns3::WifiMacQueue::MaxSize", QueueSizeValue(QueueSize(PACKETS, 6000)));
    }
    ...
    if (phyModel == "Spectrum")
    {
        ...
        phy.EnablePcap("scratch/STA_pcap", staDevices);
        phy.EnablePcap("scratch/AP_pcap", apDevice);
    }
    ...
    ApplicationContainer serverApps;
    auto serverNodes = downlink ? std::ref(wifiApNode) : std::ref(wifiStaNodes);
    Ipv4InterfaceContainer serverInterfaces;
    NodeContainer clientNodes;
    for (std::size_t i = 0; i < nStations; i++)
    {
        serverInterfaces.Add(downlink ? apNodeInterface.Get(0) : staNodeInterfaces.Get(i));
        clientNodes.Add(downlink ? wifiStaNodes.Get(i) : wifiApNode.Get(0));
    }

    createMyServer(serverNodes);
    ...
    // httpVariables->SetReadingTimeMean(Seconds(1));
    // httpVariables->SetNumOfEmbeddedObjectsMax(1000);
    //}
    //std::cout << httpVariables->GetNumOfEmbeddedObjects() << std::endl;
    serverApps.Start(Seconds(0.0));
    serverApps.Stop(Seconds(simulationTime + 1));
    ...
    //     // std::cout << clientApps.GetN() << " Hellc" << std::endl;
    // }
    createMyClient(nStations,serverInterfaces,simulationTime,clientNodes);
    ...
    //std::cout << " Helloewfoejfej" << std::endl;


    // After the simulation
    float totalPageLoadTime = 0;
    Time totalLatency = Seconds(0);

    for (size_t i = 0; i < generationTimes.size(); i++)
    {
        totalLatency += receptionTimes[i] - generationTimes[i];
    }

    for (size_t i = 0; i < pageLoadTimes.size(); i++)
    {
        totalPageLoadTime += pageLoadTimes[i];
        //std::cout << "ver: " << pageLoadTimes[i] << std::endl;
    }

    // Calculate average metrics
    uint32_t numberOfMainObjects = generationTimes.size();
    float averagePageLoadTime = totalPageLoadTime / pageLoadTimes.size();
    Time averageLatency = totalLatency / numberOfMainObjects;

    
    std::cout << "Main Object Average Latency: " << averageLatency << std::endl;
    std::cout << "Page Load Time(ms): " << averagePageLoadTime << std::endl;
